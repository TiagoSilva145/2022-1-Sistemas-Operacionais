...
{
    return (clock() - atual->t_inicio) / 1000;
}
...
void dispatcher()
{
    ...
    printf("Task %d exit: execution time %d ms, processor time %d ms, %d activations\n",
            disp.id, systime(), disp.t_processamento, disp.ativacoes);
...
void ppos_init ()
{
   ...
   atual = &ContextMain;
   queue_append((queue_t**)&inicio_fila, (queue_t*)&ContextMain);
...
void task_exit (int exit_code)
{
    ...
    printf("Task %d exit: execution time %d ms, processor time %d ms, %d activations\n",
            atual->id, systime(), atual->t_processamento, atual->ativacoes);
...
void task_yield ()
{
    atual->t_processamento += (clock() - t_volta) / 1000;
    atual->ativacoes += 1;
    disp.ativacoes += 1;
    task_switch(&disp);
}